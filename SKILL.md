...
- Use the Custom Provider Tool Exposure triage when MCP tools or Computer Use are unavailable only under a custom `model_provider`: the agent reports zero MCP tool names, while the same machine and config expose every MCP tool with `-c model_provider=openai`. Capture the real provider request body through a local logging reverse proxy and check the `/models` response shape before touching Desktop: a gateway that answers the OpenAI list-models shape (`{"data":[...]}`) fails catalog decoding (`missing field \`models\``) and forces fallback model metadata with `supports_search_tool: false`, pinning the wire shape to `namespace` entries; a chain that passes only standard `function` tools makes both the `namespace` and `tool_search` shapes ineffective silently with HTTP 200. Treat the dropping hop (gateway filter versus model-side rejection of the private shapes) as unverified unless a controlled raw-request probe or a gateway inbound/outbound comparison proves it, and never use the model's verbal tool report as evidence. This layer sits below every Desktop patch, so neither the MSIX repatch nor the Computer Use local repair can fix it — see the custom provider chain case in `references/restriction-debug-cases.md`.
- Use the CUA surface lock case when `cua.getState()` succeeds but native-app methods are missing (`cua.getApp is not a function`, no `computer`/`getApp`/`listApps` in `Object.keys(cua)`) while `scripts\install-computer-use-local.ps1 -StrictVerifyOnly` passes and the `codex-computer-use-*` pipe exists: the materialized plugin cache `.mcp.json` pins `CUA_REPL_ENABLED_SURFACES` to `browser`. Flip it to `browser,computer`, start a fresh conversation, and require the three verification signals before any heavier workflow — see the surface lock case in `references/restriction-debug-cases.md`.
- Use the config-rewriter case when Computer Use breaks immediately after an external tool (for example a provider switcher such as CC Switch) rewrote `config.toml` and `codex mcp list` no longer lists `cua_repl` at all: the rewriter rebuilt the file and dropped `[plugins."unified-computer-use@openai-bundled"]`, sibling plugin tables, and the `computer_use`/`js_repl`/`non_prefixed_mcp_tool_names` feature keys. Restore those tables from the config backup instead of repatching, and diff the three points after every provider switch — see the config rewriter case in `references/restriction-debug-cases.md`.
...
It also forces the configured custom model IDs through the Desktop model visibility filter. By default these are `gpt-6-astra`, `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`. This patch only unhides catalog entries that already exist for the current build; it never creates one. A slug the active catalog does not contain stays absent from the picker no matter how often the bundle is repatched, so confirm the entry exists before reading a missing model as a patch failure. The CLI builtin catalog is embedded in `codex.exe` and ships not-yet-rolled-out models with `"visibility": "hide"`, which is exactly the shape this patch is for; a user `model_catalog_json` replaces that builtin catalog instead of extending it, so it must carry the new entry itself. On builds containing the compact Power slider, it also opens the Electron-specific `harborEnabled` gate so the matching GPT-5.6 model and reasoning combinations use that slider instead of the legacy model/effort/speed-only menu.
...
- `-CustomModels <id1,id2,...>`: custom model IDs forced through the Desktop model visibility filter; defaults to `gpt-6-astra`, `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`. Passing this parameter replaces the default list rather than extending it, so repeat the models you still want alongside the new one.
