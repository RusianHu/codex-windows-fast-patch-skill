...
The mode requires exactly one content-matched `.vite\build` target and one occurrence of each original gate. Idempotency requires both complete patched gates, one `CODEX_CUA_WINDOWS_SURFACE_V1` marker, and no original gates; a marker alone, mixed state, or duplicate candidate fails closed. The Windows branch requires `computerUse`; Desktop 26.917 removed the `computerUseNodeRepl` feature flag, so the guarded patch accepts both the older two-flag shape and the newer one-flag shape. Darwin behavior remains unchanged. The patched asset must pass `node --check`. This mode skips the unrelated Chrome registry patch and rejects combinations with other targeted modes, marketplace registration, or Fast Mode verification. It does not edit plugin cache files, `.mcp.json`, or user configuration. Normal repacking still updates the copied package's ASAR integrity metadata and signature; installation/relaunch must run from an external executor.
